...
class APIManager {
    ...
    async loadAPIConfig() {
        ...
        this.isDemoMode = (this.apiKey === 'demo-api-key' && this.shopId === 'demo-shop-id');
        
        // Also check localStorage for demo credentials
        try {
            const savedCredentials = localStorage.getItem('clutch_credentials');
            if (savedCredentials) {
                const credentials = JSON.parse(savedCredentials);
                if (credentials.apiKey === 'demo-api-key' && credentials.shopId === 'demo-shop-id') {
        ...
        if (this.isDemoMode) {
            console.log('API Manager running in demo mode');
        }
    }
    ...
    async testConnection() {
        try {
            // Skip actual API calls in demo mode
            if (this.isDemoMode) {
    ...
    getMockData(endpoint, method, data) {
        ...
        const mockResponses = {
            ...
            '/system/health': { status: 'ok', timestamp: new Date().toISOString() },
            '/system/ping': { pong: true, timestamp: new Date().toISOString() },
            '/system/time': { time: new Date().toISOString() },
            '/system/version': { version: '1.0.0', build: 'demo' },
            '/auth/verify': { valid: true, shop_id: 'demo-shop-id' },
            '/shops/profile': { 
                id: 'demo-shop-id', 
                name: 'متجر تجريبي', 
            ...
            '/sales/transactions': { transactions: [], total: 0 },
            '/sales/analytics': { analytics: {} },
            '/market/top-selling': { items: [] },
            '/market/popular-cars': { cars: [] },
            '/market/trends': { trends: [] },
            '/market/insights': { insights: {} },
            '/ai/demand-forecast': { forecast: {} },
            '/ai/market-analysis': { analysis: {} },
            '/ai/inventory-optimization': { optimization: {} },
            '/ai/price-optimization': { optimization: {} },
            '/ai/customer-insights': { insights: {} }
        };
        
        // Return mock data or empty object
        return mockResponses[endpoint] || { success: true, data: null };
    }
