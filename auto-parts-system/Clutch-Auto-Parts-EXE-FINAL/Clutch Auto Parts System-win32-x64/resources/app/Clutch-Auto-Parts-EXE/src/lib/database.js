...
class DatabaseManager {
    constructor() {
        this.db = null;
        this.config = {
            filename: path.join((0, utils_1.getDataPath)(), 'clutch_auto_parts.db'),
        ...
        return new Promise((resolve, reject) => {
            this.db = new sqlite3.Database(this.config.filename, this.config.mode, (err) => {
                if (err) {
                    ...
                    logger_1.logger.info('Database opened successfully');
                    this.createTables()
                        .then(() => resolve())
                        .catch(reject);
                }
    ...
    async insertDefaultData() {
        ...
        await this.exec('INSERT INTO users (username, email, password_hash, role, first_name, last_name) VALUES (?, ?, ?, ?, ?, ?)', ['admin', 'admin@clutch.com', hashedPassword, 'owner', 'System', 'Administrator']);
        ...
        for (const [name, nameAr, desc, descAr] of categories) {
            await this.exec('INSERT INTO categories (name, name_ar, description, description_ar) VALUES (?, ?, ?, ?)', [name, nameAr, desc, descAr]);
        }
        ...
        for (const [name, nameAr] of brands) {
            await this.exec('INSERT INTO brands (name, name_ar) VALUES (?, ?)', [name, nameAr]);
        }
        ...
        for (const [key, value, description] of settings) {
            await this.exec('INSERT INTO settings (key, value, description) VALUES (?, ?, ?)', [key, value, description]);
        }
    ...
    async query(sql, params = []) {
        return new Promise((resolve, reject) => {
            ...
            this.db.all(sql, params, (err, rows) => {
                if (err) {
                    logger_1.logger.error('Database query error:', err);
                    reject(err);
                }
                else {
                    resolve(rows || []);
    ...
    async exec(sql, params = []) {
        return new Promise((resolve, reject) => {
            ...
            this.db.run(sql, params, function (err) {
                if (err) {
                    logger_1.logger.error('Database exec error:', err);
                    reject(err);
                }
                else {
                    resolve(this);
    ...
    async get(sql, params = []) {
        return new Promise((resolve, reject) => {
            ...
            this.db.get(sql, params, (err, row) => {
                if (err) {
                    logger_1.logger.error('Database get error:', err);
                    reject(err);
                }
                else {
                    resolve(row);
    ...
    async backup(backupPath) {
        return new Promise((resolve, reject) => {
            ...
            this.db.backup(backupPath, (err) => {
                if (err) {
                    logger_1.logger.error('Database backup error:', err);
                    reject(err);
                }
                else {
