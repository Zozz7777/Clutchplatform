# PowerShell script to update all legacy UI components to SnowUI
# This script will systematically replace Button, Card, and Input components

Write-Host "Starting SnowUI migration..." -ForegroundColor Green
...
$filesToUpdate = Get-ChildItem -Recurse -Include "*.tsx" | Where-Object {
    ...
    return $content -match "import.*Button.*from.*@/components/ui/button" -or
           ...
           $content -match "import.*Input.*from.*@/components/ui/input"
}
...
Write-Host "SnowUI migration completed!" -ForegroundColor Green
