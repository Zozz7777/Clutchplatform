...
export function LuxurySidebar({ 
  ...
  // Premium Navigation Items with Luxury Icons
  ...
  return (
    <>
      {/* Luxury Main Sidebar */}
      <div
        className={`fixed left-0 top-0 z-40 h-screen bg-gradient-to-b from-white/90 via-white/80 to-white/90 backdrop-blur-xl border-r border-white/20 shadow-2xl transition-all duration-500 ${
          selectedParent ? 'w-16' : (sidebarCollapsed ? 'w-16' : 'w-64')
        }`}
      >
        {/* Luxury Sidebar Header */}
        <div className="flex h-16 items-center justify-between px-4 border-b border-white/20 bg-gradient-to-r from-luxury-gold-500/10 to-luxury-diamond-500/10 backdrop-blur-md">
          {!sidebarCollapsed && !selectedParent && (
            <div className="flex items-center space-x-3">
              ...
              <div>
                <span className="text-xl font-bold bg-gradient-to-r from-luxury-gold-600 to-luxury-diamond-600 bg-clip-text text-transparent">
                  Clutch
                </span>
                <div className="text-xs text-luxury-gold-600 font-medium">Premium Edition</div>
              </div>
            </div>
          )}
          <LuxuryButton
        ...
        {/* Luxury Navigation */}
        <nav className="flex-1 overflow-y-auto py-6 mt-16 px-3">
                ...
                <div key={category} className="space-y-2">
                  ...
                  {!sidebarCollapsed && !selectedParent && (
                    <button
                      ...
                      className="w-full flex items-center justify-between px-3 py-2 text-xs font-bold text-luxury-gold-600 uppercase tracking-wider hover:text-luxury-gold-700 transition-all duration-300 hover:scale-105"
                    >
                      ...
                      <ChevronDown 
                        className={`h-3 w-3 transition-all duration-300 ${
                          isExpanded ? 'rotate-180 text-luxury-diamond-500' : 'text-luxury-gold-500'
                        }`} 
                      />
                    </button>
                  )}
                    ...
                    <div className="space-y-1">
                      {items.map((item) => {
                        ...
                        return (
                          <div key={item.href}>
                            <LuxuryButton
                              variant={isActive || isSelected ? 'luxury' : item.luxury ? 'glass' : 'glass'}
                              size="sm"
                              ...
                              className={`w-full justify-start group transition-all duration-300 ${
                                isActive || isSelected
                                  ? 'shadow-luxury-lg scale-105'
                                  : 'hover:scale-105 hover:shadow-lg'
                              } ${item.luxury ? 'border border-luxury-gold-200' : ''}`}
                              shimmer={isHovered}
                              glow={isActive}
                            >
                              <item.icon className={`h-5 w-5 mr-3 transition-all duration-300 ${
                                isActive || isSelected 
                                  ...
                                  : item.luxury 
                                    ? 'text-luxury-gold-600 group-hover:text-luxury-gold-700 group-hover:scale-110'
                                  ...
                                  {hasChildren && (
                                    <ChevronDown 
                                      className={`h-4 w-4 transition-all duration-300 ${
                                        isSelected ? 'rotate-180 text-white' : 'text-slate-500 group-hover:text-slate-700'
                                      }`} 
                                    />
                                  )}
                                  {item.badge && (
                                    <span className={`ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-bold ${
                                      item.badge === 'NEW' ? 'bg-gradient-to-r from-luxury-emerald-500 to-luxury-emerald-600 text-white shadow-lg' :
                                      item.badge === 'AI' ? 'bg-gradient-to-r from-luxury-diamond-500 to-luxury-diamond-600 text-white shadow-lg' :
                                      item.badge === 'PREMIUM' ? 'bg-gradient-to-r from-luxury-gold-500 to-luxury-gold-600 text-white shadow-lg' :
                                      item.badge === 'B2B' ? 'bg-gradient-to-r from-luxury-platinum-500 to-luxury-platinum-600 text-white shadow-lg' :
                                      item.badge === 'SEC' ? 'bg-gradient-to-r from-luxury-ruby-500 to-luxury-ruby-600 text-white shadow-lg' :
        ...
        {/* Luxury User Section */}
        {!sidebarCollapsed && !selectedParent && (
          <div className="border-t border-white/20 p-4 bg-gradient-to-r from-luxury-gold-500/5 to-luxury-diamond-500/5 backdrop-blur-md">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 bg-gradient-to-br from-luxury-gold-500 to-luxury-diamond-500 rounded-xl flex items-center justify-center shadow-lg">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-1 mt-1">
                  ...
                  <span className="text-xs text-luxury-gold-600 font-bold">VIP Member</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* Luxury Sub-sidebar */}
      {selectedParent && (
        <div className="fixed left-16 top-0 z-30 h-screen w-64 bg-gradient-to-b from-white/90 via-white/80 to-white/90 backdrop-blur-xl border-r border-white/20 shadow-2xl transition-all duration-500">
          <div className="flex h-full flex-col">
            <div className="border-b border-white/20 p-4 mt-16 bg-gradient-to-r from-luxury-gold-500/10 to-luxury-diamond-500/10 backdrop-blur-md">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-luxury-gold-700 flex items-center gap-2">
              ...
              <div className="space-y-1">
                {navigationItems.find(item => item.href === selectedParent)?.children?.map((child) => {
                  ...
                  return (
                    <LuxuryButton
                      key={child.href}
                      variant={isChildActive ? 'luxury' : 'glass'}
                      size="sm"
                      onClick={() => router.push(child.href)}
                      className={`w-full justify-start transition-all duration-300 ${
                        isChildActive ? 'shadow-luxury-lg scale-105' : 'hover:scale-105'
                      }`}
                      glow={isChildActive}
                    >
                      {child.title}
