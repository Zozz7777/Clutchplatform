...
/**
 * Direct MongoDB Database Audit Script
 * Bypasses complex environment configuration for direct database access
 */
    ...
    if (missingCollections.length > 0) {
      ...
      for (const collectionName of missingCollections) {
        try {
          ...
          if (collectionName.includes('mobile_app')) {
            await collection.createIndex({ version: 1 });
            await collection.createIndex({ platform: 1 });
          }
          
          if (collectionName.includes('marketing') || collectionName === 'campaigns') {
            await collection.createIndex({ status: 1 });
            await collection.createIndex({ type: 1 });
          }
          
          if (collectionName.includes('ai_') || collectionName === 'anomalies' || collectionName === 'fraud_cases') {
            await collection.createIndex({ type: 1 });
            await collection.createIndex({ status: 1 });
          }
          
          if (collectionName.includes('security_') || collectionName === 'threat_intelligence') {
            await collection.createIndex({ type: 1 });
            await collection.createIndex({ severity: 1 });
          }
          
          if (collectionName.includes('asset') || collectionName === 'maintenance') {
            await collection.createIndex({ assetId: 1 });
            await collection.createIndex({ status: 1 });
          }
