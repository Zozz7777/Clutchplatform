...
router.get('/logs', authenticateToken, requireRole(['admin', 'auditor']), auditRateLimit, async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 50, 
...
router.get('/logs/:id', authenticateToken, requireRole(['admin', 'auditor']), auditRateLimit, async (req, res) => {
  try {
    const { id } = req.params;
    const auditLogsCollection = await getCollection('audit_logs');
...
router.post('/logs', authenticateToken, requireRole(['admin', 'system']), auditRateLimit, async (req, res) => {
  try {
    const {
      userId,
      action,
...
router.get('/user-activity/:userId', authenticateToken, requireRole(['admin', 'auditor']), auditRateLimit, async (req, res) => {
  try {
...
router.get('/security-events', authenticateToken, requireRole(['admin', 'security']), auditRateLimit, async (req, res) => {
  try {
    const { 
      page = 1, 
      limit = 50, 
...
router.post('/security-events', authenticateToken, requireRole(['admin', 'system']), auditRateLimit, async (req, res) => {
  try {
    const {
      type,
      severity,
...
router.get('/compliance-report', authenticateToken, requireRole(['admin', 'compliance']), auditRateLimit, async (req, res) => {
  try {
    const { 
      startDate, 
      endDate, 
...
router.get('/analytics', authenticateToken, requireRole(['admin', 'auditor']), auditRateLimit, async (req, res) => {
  try {
...
router.get('/', authenticateToken, requireRole(['admin', 'auditor']), auditRateLimit, async (req, res) => {
  res.json({
    success: true,
